; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -disable-kvx-hwloops=false -o - %s -O2 | FileCheck %s --check-prefixes=CHECK
; RUN: llc -disable-kvx-hwloops=false -mcpu=kv3-2 -o - %s -O2 | FileCheck %s --check-prefixes=CHECK
; RUN: clang -mllvm -disable-kvx-hwloops=false -O2 -c -o /dev/null %s
; RUN: clang -march=kv3-2 -mllvm -disable-kvx-hwloops=false -O2 -c -o /dev/null %s

target triple = "kvx-kalray-cos"

@x = common global ptr null, align 8

define ptr @f(i32 %s, i32 %c){
; CHECK-LABEL: f:
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    make $r0 = x
; CHECK-NEXT:    make $r1 = 0
; CHECK-NEXT:    make $r2 = 126
; CHECK-NEXT:    ;; # (end cycle 0)
; CHECK-NEXT:    ld $r0 = 0[$r0]
; CHECK-NEXT:    ;; # (end cycle 1)
; CHECK-NEXT:    loopdo $r2, .__LOOPDO_0_END_
; CHECK-NEXT:    ;;
; CHECK-NEXT:  .LBB0_1: # %for.body
; CHECK-NEXT:    # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    addd $r1 = $r1, 4
; CHECK-NEXT:    addx4d $r2 = $r1, $r0
; CHECK-NEXT:    addd $r3 = $r1, 0xfffffc18
; CHECK-NEXT:    ;; # (end cycle 0)
; CHECK-NEXT:    sw 0xfffffffffffff060[$r2] = $r3
; CHECK-NEXT:    ;; # (end cycle 1)
; CHECK-NEXT:  .__LOOPDO_0_END_:
; CHECK-NEXT:  # %bb.2: # %for.body.split
; CHECK-NEXT:    ret
; CHECK-NEXT:    ;;
entry:
  %0 = load ptr, ptr @x, align 8
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body
  ret ptr %0

for.body:                                         ; preds = %entry, %for.body
  %indvars.iv = phi i64 [ -1000, %entry ], [ %indvars.iv.next, %for.body ]
  %arrayidx = getelementptr inbounds i32, ptr %0, i64 %indvars.iv
  %1 = trunc i64 %indvars.iv to i32
  store i32 %1, ptr %arrayidx, align 4
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 4
  %cmp = icmp slt i64 %indvars.iv, -503
  br i1 %cmp, label %for.body, label %for.cond.cleanup
}

define ptr @g(i32 %s, i32 %c){
; CHECK-LABEL: g:
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    make $r0 = x
; CHECK-NEXT:    make $r1 = 0
; CHECK-NEXT:    make $r2 = 125
; CHECK-NEXT:    ;; # (end cycle 0)
; CHECK-NEXT:    ld $r0 = 0[$r0]
; CHECK-NEXT:    ;; # (end cycle 1)
; CHECK-NEXT:    loopdo $r2, .__LOOPDO_1_END_
; CHECK-NEXT:    ;;
; CHECK-NEXT:  .LBB1_1: # %for.body
; CHECK-NEXT:    # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    addd $r1 = $r1, 4
; CHECK-NEXT:    addx4d $r2 = $r1, $r0
; CHECK-NEXT:    addd $r3 = $r1, 0xfffffc18
; CHECK-NEXT:    ;; # (end cycle 0)
; CHECK-NEXT:    sw 0xfffffffffffff060[$r2] = $r3
; CHECK-NEXT:    ;; # (end cycle 1)
; CHECK-NEXT:  .__LOOPDO_1_END_:
; CHECK-NEXT:  # %bb.2: # %for.body.split
; CHECK-NEXT:    ret
; CHECK-NEXT:    ;;
entry:
  %0 = load ptr, ptr @x, align 8
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body
  ret ptr %0

for.body:                                         ; preds = %entry, %for.body
  %indvars.iv = phi i64 [ -1000, %entry ], [ %indvars.iv.next, %for.body ]
  %arrayidx = getelementptr inbounds i32, ptr %0, i64 %indvars.iv
  %1 = trunc i64 %indvars.iv to i32
  store i32 %1, ptr %arrayidx, align 4
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 4
  %cmp = icmp slt i64 %indvars.iv, -504
  br i1 %cmp, label %for.body, label %for.cond.cleanup
}

