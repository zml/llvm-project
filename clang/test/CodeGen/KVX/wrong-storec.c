...
void wrong_ready(s128 *s, v8i64 v, int sv, void * addr, int ready, unsigned long cond){
  ...
  __builtin_kvx_storecw(sv, addr, cond, ".toto"); // expected-error {{Expects a lsucond, one of: '.dgtz', '.wgtz', '.dltz', '.even', '.dlez', '.dnez', '.wltz', '.odd', '.wnez', '.dgez', '.wlez', '.deqz', '.weqz', '.wgez'}} expected-error {{cannot compile this builtin function yet}}
}
