...
// REQUIRES: librt_has_udivmodv4si4

#include <stdio.h>

// Returns: a / b, if (r) r = a % b

typedef unsigned v4su __attribute__((__vector_size__(16)));
v4su __udivmodv4si4(v4su a, v4su b, v4su *r);

int test__udivmodv4si4(v4su a, v4su b, v4su expected_q, v4su expected_rem) {
  v4su rem;
  v4su q = __udivmodv4si4(a, b, &rem);
  int res = 0;
  for (int lane = 0; lane < 4; ++lane)
    if (q[lane] != expected_q[lane] || rem[lane] != expected_rem[lane]) {
      printf("error in __udivmodv4si4 (lane %d): %X divmod %X = q(%X), rem(%X), "
             "expected q(%X), rem (%X)\n",
...
int main() {
  ...
  unsigned i;
  for (i = 0; i < N; ++i)
    if (test__udivmodv4si4(tests[i][0], tests[i][1], tests[i][2], tests[i][3]))
      return 1;
