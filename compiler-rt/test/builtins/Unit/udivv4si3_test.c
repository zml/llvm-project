...
v4su __udivv4si3(v4su a, v4su b);



int test__udivv4si3(v4su a, v4su b, v4su expected_r)
{
    v4su r = __udivv4si3(a, b);
    int res = 0;
    for (int lane = 0; lane < 4; ++lane)
      if (r[lane] != expected_r[lane]) {
        printf("error in __udivv4si3 (lane %d): %X / %X = %X, expected %X\n",
...
int main()
{
    ...
    unsigned i;
    for (i = 0; i < N; ++i)
        if (test__udivv4si3(tests[i][0], tests[i][1], tests[i][2]))
            return 1;
