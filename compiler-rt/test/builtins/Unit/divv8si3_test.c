...
vt __divv8si3(vt a, vt b);

int test__divv8si3(vt a, vt b, vt expected) {
  const int N = sizeof(vt) / sizeof(elt);
  vt q = __divv8si3(a, b);
  int res = 0;
  for (int lane = 0; lane < N; ++lane)
    if (q[lane] != expected[lane]) {
      printf("error in __divv8si3 (lane %d): %X / %X = %X, "
             "expected %X\n",
...
int main() {
  ...
  unsigned i;
  for (i = 0; i < N; ++i)
    if (test__divv8si3(tests[i][0], tests[i][1], tests[i][2]))
      return 1;
